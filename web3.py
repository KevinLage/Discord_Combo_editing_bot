...
def index():
    ...
    while not dic[fielcode]:
        time.sleep(3)
    os.system("zip " + fielcode + '_edited.zip ' + fielcode +"_edited.txt") #creates a zip file

    
    files = {
        'file': ('./' + fielcode + '_edited.zip', open('./' + fielcode + '_edited.zip', 'rb')),}
    response = requests.post('https://x0.at/', files=files) #upload the files to x0.at
    try:
        os.remove("./" + fielcode + ".txt")     #deletes all files
...
if __name__ == '__main__':    app.run(port=1338)